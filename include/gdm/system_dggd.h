...
namespace GDM
{
  ...
  namespace internal
  {
    ...
    public:
      ...
      neighbor(const unsigned int f) const
      {
        ...
        // DGGD has one extra cell per direction.
        const auto shape = system.cell_shape();
      ...
      get_dof_indices(std::vector<types::global_dof_index> &dof_indices) const
      {
        ...
        // Find the first primal node of the cell stencil in each direction.
        for (unsigned int d = 0; d < dim; ++d)
          offset_reference[d] =
            (indices[d] < system.fe_degree / 2) ?
              0 :
              std::min(system.n_subdivisions[d] - system.fe_degree,
                       indices[d] - system.fe_degree / 2);
                            ...
                            const MPI_Comm           comm)
    {
      ...
      const auto partitioner =
        std::make_shared<dealii::Utilities::MPI::Partitioner>(
          dof_handler.locally_owned_dofs(), active_dofs, comm);
  ...
  {
  public:
    // Validate the degree before constructing the finite element collection.
    static unsigned int
    validate_degree(const unsigned int degree)
    {
      AssertThrow(degree >= 2 && degree % 2 == 0,
    ...
    System(const unsigned int fe_degree,
      ...
      : comm(MPI_COMM_NULL)
      , fe_degree(validate_degree(fe_degree))
      , n_components(n_components)
      , fe(generate_fe_collection<dim>(generate_polynomials_1D(this->fe_degree),
                                       n_components))
      , add_ghost_layer(add_ghost_layer)
    {
      AssertThrow(fe.size() == Utilities::pow(this->fe_degree + 1, dim),
                  ExcMessage("generate_fe_collection() built a different "
                             "number of basis variants than expected."));
    }
    ...
    // Build the primal helper mesh and the DGGD integration mesh.
    void
    subdivided_hyper_rectangle(const std::vector<unsigned int> &n_subdivisions,
                               const Point<dim>                &p1,
                               const Point<dim>                &p2)
    {
      AssertDimension(n_subdivisions.size(), dim);

      for (unsigned int d = 0; d < dim; ++d)
        {
          AssertThrow(n_subdivisions[d] >= fe_degree,
                      ExcMessage("DGGD needs at least as many subdivisions (N) "
                                 "as the polynomial degree (p): N >= p."));

          this->n_subdivisions[d] = n_subdivisions[d];
        }

      // Build the uniform primal mesh used by the helper DoFHandler.
      auto new_dof_tria = std::make_shared<Triangulation<dim>>();

      GridGenerator::subdivided_hyper_rectangle(
        *new_dof_tria, n_subdivisions, p1, p2, true);

      create_triangulation_pre();

      // DGGD cell widths are h/2, h, ..., h, h/2.
      std::vector<std::vector<double>> step_sizes(dim);

      for (unsigned int d = 0; d < dim; ++d)
        {
          const double h = (p2[d] - p1[d]) / n_subdivisions[d];

          step_sizes[d].push_back(h / 2.0);

          for (unsigned int i = 1; i < n_subdivisions[d]; ++i)
            step_sizes[d].push_back(h);

          step_sizes[d].push_back(h / 2.0);
        }

      GridGenerator::subdivided_hyper_rectangle(
        *tria, step_sizes, p1, p2, true);

      create_triangulation_post(new_dof_tria);
    }

    // Use the same number of subdivisions in every direction.
    void
    subdivided_hyper_cube(const unsigned int n_subdivisions_1D,
                          const double       left  = 0.0,
                          const double       right = 1.0)
    {
      std::vector<unsigned int> n_subdivisions(dim, n_subdivisions_1D);
      ...
      subdivided_hyper_rectangle(n_subdivisions, p1, p2);
    }
    ...
    categorize()
    {
      ...
      // Even-degree DGGD has p+1 position-dependent basis categories.
      const unsigned int category_count = fe_degree + 1;

      for (unsigned int i = 0; i < active_cell_index_map.size(); ++i)
        {
          unsigned int cell_index = active_cell_index_map[i];

          auto indices = index_to_indices<dim>(cell_index, cell_shape());

          // Select the left, interior, or right stencil category.
          for (unsigned int d = 0; d < dim; ++d)
          ...
          active_fe_indices[i] = indices_to_index<dim>(indices, category_count);
        }
    }
    ...
    {
      for (unsigned int surface = 0; surface < 2 * dim; ++surface)
        interpolate_boundary_values(mapping, surface, fu, constraints);
    }
  ...
  private:
    ...
    create_triangulation_pre()
    {
      if (comm == MPI_COMM_NULL)
      ...
      else
        {
          ...
          // The serial helper mesh does not support distributed ownership.
          AssertThrow(n_procs == 1,
                      ExcMessage("This DGGD System does not yet support "
                                 "more than one MPI process (the helper "
                                 "DoFHandler's mesh is not "
                                 "parallel-partitioned). Run with a "
                                 "single process, or use the non-MPI "
          ...
          // Partition the N+1 DGGD cells in the last direction.
          const unsigned int cell_stride =
            (cell_shape()[dim - 1] + n_procs - 1) / n_procs;

          temp->signals.create.connect([&, cell_stride, n_procs]() {
            for (const auto &cell : tria->active_cell_iterators())
              {
                ...
                const unsigned int owner =
                  std::min(indices[dim - 1] / cell_stride, n_procs - 1);
                cell->set_subdomain_id(owner);
              }
          });
    ...
    create_triangulation_post(
      const std::shared_ptr<Triangulation<dim>> &new_dof_tria)
    {
      active_cell_index_map.clear();
      ...
      // Attach the helper DoFHandler to the new primal mesh before
      // releasing the old mesh.
      dof_handler.reinit(*new_dof_tria);
      dof_tria = new_dof_tria;

      dof_handler.distribute_dofs(FESystem<dim>(FE_Q<dim>(1), n_components));

      internal::compute_renumbering_lex(dof_handler,
                                        dof_tria->get_communicator());

      AssertThrow(dof_handler.n_dofs() == n_dofs(),
                  ExcMessage("Helper DoFHandler's DoF count does not "
                             "match System::n_dofs()."));
    }
    ...
    // Uniform primal mesh used by the helper FE_Q(1) DoFHandler.
    std::shared_ptr<Triangulation<dim>> dof_tria;

    // Number of DGGD integration cells in each direction.
    std::array<unsigned int, dim>
    cell_shape() const
    {
      std::array<unsigned int, dim> shape = n_subdivisions;
      for (unsigned int d = 0; d < dim; ++d)
        shape[d] += 1;
      return shape;
    }
    ...
    // Allow CellAccessor to read the private mesh and indexing data.
