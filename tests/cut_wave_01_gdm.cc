     ...
     const double       cfl)
{
  ...
  const auto partitioner =
    std::make_shared<Utilities::MPI::Partitioner>(system.locally_owned_dofs(),
                                                  system.locally_relevant_dofs(
                                                    constraints),
                                                  comm);
  BlockVectorType solution(3);
  solution.block(1).reinit(partitioner);
  solution.block(2).reinit(partitioner);
  ...
  const auto fu_rhs = [&](const double           time,
    ...
    result.block(1) = stage_bc_and_solution.block(2);
    ...
    if (solver_name == "AMG" || solver_name == "ILU")
      ...
      {
        solver_direct.solve(sparse_matrix, result.block(2), vec_rhs);
      }
    else
      {
        AssertThrow(false, ExcNotImplemented());
    ...
    // std::cout << result.block(1).l2_norm() << " " <<
    // result.block(2).l2_norm() << std::endl;
